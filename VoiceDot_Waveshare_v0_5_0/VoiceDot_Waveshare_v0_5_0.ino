...
static const char* FW_VERSION = "0.5.0-test";
...
esp_err_t audioWrite(const void *src, size_t size, size_t *bytesWritten, uint32_t timeoutMs) {
  ...
  return i2s_channel_write(audioTxChan, src, size, bytesWritten, timeoutMs);
}
...
public:
  ...
  bool ConsumeSample(int16_t sample[2]) override {
    ...
    size_t bytesWritten = 0;
    return audioWrite(sample, 2 * sizeof(int16_t), &bytesWritten, 50) == ESP_OK &&
