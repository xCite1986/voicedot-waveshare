...
#define VOICEDOT_ENABLE_MP3 1
#if VOICEDOT_ENABLE_MP3
...
static const char* FW_VERSION = "0.5.5-test";
...
static constexpr uint32_t TTS_MP3_TASK_TIMEOUT_MS = 18000;
static constexpr uint16_t TTS_MP3_TASK_STACK = 12288;
...
bool ttsTaskActive = false;
TaskHandle_t ttsTaskHandle = nullptr;
String ttsTaskUrl = "";
uint32_t ttsTaskStartedAt = 0;
...
void diagLog(const char *tag, const String &message) {
  ...
  Serial.printf("[%lu #%lu] %s: %s | wake=%s rec=%u send=%u tts=%u task=%u heap=%lu\n",
...
bool wakeCanStart() {
  return !wakeBusy && !ttsPlaybackActive && !ttsTaskActive &&
         (int32_t)(millis() - wakeIgnoreUntil) >= 0;
}
...
void pollMicLevel() {
  if (!audioI2sReady || !codecRecordReady || speakerTestActive || ttsPlaybackActive || ttsTaskActive) return;
...
bool playMp3TtsUrlBlocking(const String &url) {
#if VOICEDOT_ENABLE_MP3
  ...
  while (ok && mp3.isRunning() && (uint32_t)(millis() - started) < TTS_MP3_TASK_TIMEOUT_MS) {
...
#else
  (void)url;
  wakeTtsStatus = "MP3 Diagnose deaktiviert: ESP8266Audio wird nicht kompiliert.";
  diagLog("TTS_MP3", wakeTtsStatus);
  wakeIgnoreUntil = millis() + WAKE_COOLDOWN_MS;
  return false;
#endif
...
void mp3TtsTask(void *param) {
  (void)param;
  String url = ttsTaskUrl;
  diagLog("TTS_TASK", String("start ") + url);
  bool ok = playMp3TtsUrlBlocking(url);

  wakeLastState = ok ? "done" : "partial";
  wakeLastMessage = ok
  ...
  diagLogf("TTS_TASK", "end ok=%u cooldown=%u status=%s",
           ok ? 1 : 0,
           WAKE_COOLDOWN_MS,
           wakeTtsStatus.c_str());
  vTaskDelete(nullptr);
...
bool startMp3TtsUrl(const String &url) {
#if VOICEDOT_ENABLE_MP3
  if (ttsTaskActive || ttsPlaybackActive) {
    wakeTtsStatus = "TTS läuft bereits.";
    diagLog("TTS_TASK", wakeTtsStatus);
    return false;
  }

  ttsTaskUrl = url;
  ttsTaskActive = true;
  ttsTaskStartedAt = millis();
  wakeTtsStatus = "MP3-Wiedergabe gestartet.";
  ...
  if (created != pdPASS) {
    ttsTaskActive = false;
    ttsTaskHandle = nullptr;
    wakeTtsStatus = "MP3 Task konnte nicht gestartet werden.";
    diagLog("TTS_TASK", wakeTtsStatus);
    return false;
  }

  diagLog("TTS_TASK", "created");
  return true;
#else
  (void)url;
  wakeTtsStatus = "MP3 Diagnose deaktiviert: ESP8266Audio wird nicht kompiliert.";
...
bool playMp3TtsUrl(const String &url) {
  return startMp3TtsUrl(url);
}

void serviceTtsTaskWatchdog() {
  if (!ttsTaskActive) return;
  if ((uint32_t)(millis() - ttsTaskStartedAt) <= TTS_MP3_TASK_TIMEOUT_MS + 4000) return;

  diagLog("TTS_TASK", "watchdog timeout, stop task");
  if (ttsTaskHandle != nullptr) {
  ...
  ttsPlaybackActive = false;
  ttsTaskActive = false;
  wakeTtsStatus = "MP3 Task Timeout.";
  wakeLastState = "partial";
  wakeLastMessage = "HA Assist fertig, TTS nicht abgespielt: " + wakeTtsStatus;
  wakeIgnoreUntil = millis() + WAKE_COOLDOWN_MS;
  if (apMode) ledsStatusSetup();
  else ledsStatusReady();
}
...
bool sendAudioToHaAssist(const uint8_t *audio, size_t bytes) {
  ...
  if (ok) {
    if (wakeTtsUrl.length() > 0) {
      ...
      if (ttsTaskActive) {
        wakeLastState = "tts";
        wakeLastMessage = "HA Assist fertig, TTS läuft.";
      } else {
...
void runWakeCaptureAndHa() {
  ...
  wakeBusy = false;
  diagLogf("WAKE", "end ok=%u cooldown=%u state=%s ttsTask=%u",
           ok ? 1 : 0,
           WAKE_COOLDOWN_MS,
           wakeLastState.c_str(),
           ttsTaskActive ? 1 : 0);
...
"   'TTS TASK    '+(j.assist.tts_task?'RUNNING':'-')+'\\n'+\n"
...
void handleStatus() {
  ...
  json += "\"tts_task\":" + String(ttsTaskActive ? "true" : "false");
  json += "}";
...
void loop() {
  serviceTtsTaskWatchdog();

  static uint32_t lastLoopDiag = 0;
  if (DEBUG_SERIAL && (wakeBusy || wakeRecording || wakeSending || ttsPlaybackActive || ttsTaskActive) &&
