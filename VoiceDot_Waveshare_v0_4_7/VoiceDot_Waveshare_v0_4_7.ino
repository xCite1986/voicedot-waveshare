/*
  ...
  IMPORTANT:
    This version deliberately focuses on a reliable board bring-up:
      ...
      - first Home Assistant TTS WAV/PCM and optional MP3 playback over ES8311
      - orange rotating wait/thinking LED ring

  Arduino IDE dependency:
    - Adafruit NeoPixel
    - ESP8266Audio for MP3 TTS playback
...
#include <driver/i2s.h>
  ...
  #define VOICEDOT_HAS_MP3 1
#else
  #define VOICEDOT_HAS_MP3 0
#endif
#include <ctype.h>
#include <math.h>
...
static const char* FW_VERSION = "0.4.7-test";
...
public:
  ...
  bool SetRate(int hz) override {
    ...
    currentRate = hz;
    return i2s_set_clk(AUDIO_I2S_PORT, (uint32_t)hz,
  ...
  bool ConsumeSample(int16_t sample[2]) override {
    ...
    size_t bytesWritten = 0;
    return i2s_write(AUDIO_I2S_PORT, sample, 2 * sizeof(int16_t),
                     &bytesWritten, pdMS_TO_TICKS(50)) == ESP_OK &&
  ...
  uint16_t ConsumeSamples(int16_t *samples, uint16_t count) override {
    ...
    while (done < count) {
      ...
      if (i2s_write(AUDIO_I2S_PORT, out, bytesToWrite,
                    &bytesWritten, pdMS_TO_TICKS(250)) != ESP_OK ||
  ...
  bool stop() override {
    i2s_zero_dma_buffer(AUDIO_I2S_PORT);
    i2s_set_clk(AUDIO_I2S_PORT, AUDIO_SAMPLE_RATE, I2S_BITS_PER_SAMPLE_16BIT, I2S_CHANNEL_STEREO);
...
bool playWavPcm16(Stream &stream) {
  ...
  es8311SetMute(false);
  es8311SetVolume(cfg.volume > 0 ? cfg.volume : 60);
  i2s_set_clk(AUDIO_I2S_PORT, sampleRate, I2S_BITS_PER_SAMPLE_16BIT, I2S_CHANNEL_STEREO);
  i2s_zero_dma_buffer(AUDIO_I2S_PORT);
  ...
  while (remaining > 0) {
    ...
    if (channels == 1) {
      ...
      size_t bytesWritten = 0;
      i2s_write(AUDIO_I2S_PORT, stereoBuf, samples * 2 * sizeof(int16_t),
                &bytesWritten, pdMS_TO_TICKS(250));
    } else {
      size_t bytesWritten = 0;
      i2s_write(AUDIO_I2S_PORT, inBuf, chunk, &bytesWritten, pdMS_TO_TICKS(250));
    }
...
bool playMp3TtsUrl(const String &url) {
#if !VOICEDOT_HAS_MP3
  (void)url;
  wakeTtsStatus = "ESP8266Audio fehlt fuer MP3.";
  return false;
#else
  ...
  while (ok && mp3.isRunning() && (uint32_t)(millis() - started) < 30000) {
    if (!mp3.loop()) break;
    waitRingTick(millis(), false);
    server.handleClient();
    yield();
  ...
  bool finished = ok && !mp3.isRunning();
  mp3.stop();
  buffer.close();
  file.close();
...
bool wsConnectHa(Client &client, const String &host, uint16_t port, const String &token) {
  ...
  wakeLastWsDetail = msg.substring(0, min(180, (int)msg.length()));
  if (msg.indexOf("auth_ok") < 0) return false;
