...
static const char* FW_VERSION = "0.5.6-test";
...
bool playMp3TtsUrl(const String &url) {
  ...
  while ((uint32_t)(millis() - started) < TTS_MP3_TIMEOUT_MS) {
    ...
    if (rate != currentRate) {
      currentRate = rate;
      audioSetSampleRate((uint32_t)currentRate);
      diagLogf("TTS_MP3", "rate=%d channels=%d", currentRate, channels);
    }
    ...
    size_t bytesWritten = 0;
    if (audioWrite(writeBuf, bytesToWrite, &bytesWritten, 250) == ESP_OK &&
