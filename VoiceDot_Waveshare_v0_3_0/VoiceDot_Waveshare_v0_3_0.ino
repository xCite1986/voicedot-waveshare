...
static const char* FW_VERSION = "0.3.0-test";
...
bool setupButtons() {
  ...
  Serial.println("Buttons: K1=volume up, K2=mute, K3=volume down, BOOT=LED brightness");
  return true;
}
...
void pollMicLevel() {
  ...
  if (err != ESP_OK || bytesRead < sizeof(int16_t) * 2) return;
  ...
  micLevel = constrain((int)map(micPeak, 0, 12000, 0, 100), 0, 100);
}
...
void pollButtons() {
  ...
  if (updateButton(btnMute, key2Pressed)) toggleMute();
...
<small class="help">
Diese Testversion aktiviert ES8311/ES7210 und I²S lokal. Wenn Mic-Level und Testton passen, ist die Grundlage für die Home-Assistant-Assist-Pipeline stabil.
</small>
</section>
