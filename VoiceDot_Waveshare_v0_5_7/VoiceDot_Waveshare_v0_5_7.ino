...
static const char* FW_VERSION = "0.5.7-test";
...
static constexpr uint16_t WAKE_COOLDOWN_MS = 3500;
...
static constexpr uint32_t TTS_MP3_TIMEOUT_MS = 18000;
...
bool beginTtsPlayback(uint32_t sampleRate, const char *tag) {
  ...
  bool rateOk = audioSetSampleRate(sampleRate);
  audioClearTx();
  delay(TTS_AMP_WARMUP_MS);
  audioWriteSilenceBlocks(TTS_PREROLL_BLOCKS, 80);
  diagLogf(tag, "playback prepared rate=%lu amp=%u preroll=%u rateOk=%u",
...
bool playMp3TtsUrl(const String &url) {
  ...
  while ((uint32_t)(millis() - started) < TTS_MP3_TIMEOUT_MS) {
    while (bytesIn < MP3_INPUT_BUFFER_SIZE - 1024 && stream && stream->available() > 0) {
      ...
      size_t n = stream->readBytes((char*)inBuf + bytesIn, min<size_t>(room, 1024));
      if (n == 0) break;
      bytesIn += n;
      lastReadMs = millis();
    }
    ...
    if (sync < 0) {
      ...
      if ((uint32_t)(millis() - lastReadMs) > 2500) break;
      waitRingTick(millis(), false);
      server.handleClient();
      delay(2);
      yield();
    ...
    if (!playbackPrepared) {
      ...
      currentRate = rate;
      bool rateOk = audioSetSampleRate((uint32_t)currentRate);
      audioClearTx();
    ...
    if ((uint32_t)(millis() - lastLogMs) > 1000) {
      lastLogMs = millis();
      diagLogf("TTS_MP3", "frames=%lu out=%lu err=%lu writeFail=%lu in=%lu",
  ...
  bool finished = playbackStarted && !timedOut;
  wakeTtsStatus = finished
                  ? "MP3 abgespielt: " + String(frames) + " Frames"
                  : (timedOut ? "MP3 Timeout." : "MP3-Wiedergabe abgebrochen.");
  diagLogf("TTS_MP3", "end finished=%u timedOut=%u streamEnd=%u frames=%lu out=%lu errors=%lu failures=%lu status=%s",
           finished ? 1 : 0,
           timedOut ? 1 : 0,
           streamEnded ? 1 : 0,
           (unsigned long)frames,
